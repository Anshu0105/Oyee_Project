...
exports.getLeaderboard = async (req, res) => {
  try {
    const topUsers = await User.find({})
      .sort({ aura: -1 })
      .limit(100)
      .select('_id username auraName aura auraCount avatarEmoji equippedBadge weeklyAuraGain');
    res.json(topUsers);
  } catch (err) {
...
exports.getProfile = async (req, res) => {
  try {
    ...
    res.json({
      ...
      auraCount: targetUser.auraCount || 0,
...
exports.updateUserAura = async (req, res) => {
  try {
    const { deltaAura = 0, deltaCount = 0 } = req.body;
    
    // Increment aura and auraCount
    const updatedUser = await User.findByIdAndUpdate(
      req.user.id,
      { $inc: { aura: deltaAura, auraCount: deltaCount } },
      { new: true }
    ).select('-password');
