...
const SignUpForm = () => {
  ...
  const handleSendOTP = async (e) => {
    ...
    try {
      ...
      setOtpSuccess(
        data.otpDevMode
          ? 'Dev mode: OTP is printed in the terminal running the backend (not emailed).'
...
const LoginForm = ({ onSwitchToSignup }) => {
  ...
  const emailOk = validateEmail(email);
  const resetPwVal = validatePassword(resetPass);
  ...
  const handleResetPassword = async () => {
    if (!emailOk) return setError('Enter your university email first.');
    if (resetOtp.length !== 6) return setError('Enter the 6-digit OTP from your email (or backend terminal in dev).');
    if (!resetPwVal.valid) return setError('New password must meet the same rules as sign-up (length, number, special char).');
    setError(''); setResetLoading(true);
    try {
      const res = await fetch(`${BACKEND_URL}/api/auth/reset-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, otp: resetOtp, newPassword: resetPass })
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      setResetDone(true);
  ...
  return (
    <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      {resetDone && <SuccessMsg msg="Password updated. You can log in with your new password." />}
      {forgotSent && !resetDone && (
        <>
          <SuccessMsg
            msg="Password reset OTP sent. Check your inbox (or the backend terminal in dev mode)."
          />
          <div style={{ marginTop: '12px' }}>
            <label style={labelStyle}>RESET OTP</label>
            <input
              type="text"
              inputMode="numeric"
              maxLength={6}
          ...
          <div style={{ marginTop: '12px' }}>
            ...
            <div style={{ marginTop: '8px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <Req met={resetPwVal.minLength} label="At least 6 characters" />
              <Req met={resetPwVal.hasSpecial} label="At least 1 special character (!@#$%^&*)" />
              <Req met={resetPwVal.hasNumber} label="At least 1 number (0-9)" />
            </div>
          </div>
          <button
            type="button"
            ...
            style={{ ...btnSecondary, marginTop: '8px', opacity: resetLoading ? 0.6 : 1 }}
          >
            {resetLoading ? <><Loader2 size={16} className="spin" /> Updating...</> : <>Save new password</>}
          </button>
        </>
      )}
      <InlineError msg={error} />
