      ...
      >
        {messages.map(msg => (
          <div key={msg.id} style={{ alignSelf: msg.user.includes('You') ? 'flex-end' : (msg.isSystem ? 'center' : 'flex-start'), maxWidth: '80%' }}>
            {!msg.isSystem && <div style={{ fontSize: '0.7rem', opacity: 0.5, marginBottom: '4px', textAlign: msg.user.includes('You') ? 'right' : 'left' }}>{msg.user} • {msg.time}</div>}
            ...
            {!msg.user.includes('You') && !msg.isSystem && (
