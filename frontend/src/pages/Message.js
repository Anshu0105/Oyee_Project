...
const Message = () => {
  ...
  const handleSend = async () => {
    if (input.trim()) {
      ...
      if (!isSafe) {
        setSystemMsg('your message has been restricted as you are violating messeaging rules');
      ...
      setSystemMsg(null);
      // Logic to actually send the message would go here.
          ...
          <>
              ...
              {systemMsg && (
                <div style={{ alignSelf: 'center', maxWidth: '80%' }}>
                  <div className="glass" style={{ 
                    ...
                    <strong>⚠️ System Notice: </strong>{systemMsg}
                  </div>
                </div>
              )}
            </div>
            <div style={{ padding: '20px', borderTop: '1px solid var(--glass-border)', display: 'flex', gap: '12px' }}>
              ...
              <button 
                onClick={handleSend}
                className="interactive" 
                style={{ background: 'var(--accent-primary)', border: 'none', color: 'white', padding: '0 20px', borderRadius: '4px' }}
              >
                <Send size={18} />
              </button>
            </div>
